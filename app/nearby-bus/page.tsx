...
const Map = dynamic(
  () =>
    import("@/components/nearby-bus/Map")
      .then((mod) => {
...
export default function NearbyBus() {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setLocationError(
            "Unable to retrieve your location. Please enable location services and refresh the page."
          );
        }
      );
    } else {
  ...
  return (
    <div className="h-screen w-full flex flex-col md:h-[calc(100vh-4rem)]">
            ...
            <Sheet open={isInfoOpen} onOpenChange={setIsInfoOpen}>
              <SheetTrigger asChild>
                ...
                >
                  ...
                  <Badge
                    variant="secondary"
                    className="ml-1 bg-primary/10 text-foreground border-primary/20"
                  >
                    {nearbyBuses.length}
                  </Badge>
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-[300px] sm:w-[400px] p-0 h-full">
                <InfoContent />
              </SheetContent>
            </Sheet>
          </div>
