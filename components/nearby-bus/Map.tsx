...
const Map = ({
  ...
  const createCustomIcon = useCallback((iconName: string, color: string, isSelected: boolean) => {
    ...
    return L.divIcon({
      ...
      html: `<div style="background-color: #89CFF0; border-radius: 50%; padding: 4px;">
        <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="${isSelected ? color : "white"}" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      iconSize: [size + 8, size + 8], // Increased size to account for padding
      iconAnchor: [(size + 8) / 2, size + 8], // Adjusted anchor to center with padding
    });
  }, []);
